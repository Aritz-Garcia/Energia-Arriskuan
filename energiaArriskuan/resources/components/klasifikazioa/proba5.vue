<template>
    <div class="mt-5">
        <h1  class="text-center text-2xl testu-kolore-nagusia">5. Proba</h1>
...
<script>
    ...
    export default {
        ...
        computed: {
            proba5() {
                return this.probak.filter(proba => proba.izena == "prueba5");
            },
            probaBusqueda() {
                return this.proba5Erabiltzaileak().filter(proba => proba.erabiltzailea.includes(this.testua.toLowerCase()));
            },
            utzik() {
        ...
        methods: {
            proba5Erabiltzaileak() {
                return this.proba5.map(proba => {
